...
function Show({product, variationOptions}: { product: Product, variationOptions: number[] }) {
    ...
    const {url} = usePage()
    //
    const [selectedOption, setSelectedOption] = useState<Record<number, VariationTypeOption>>([])
    //
    ...
    const computedProduct = useMemo(() => {
        ...
        const selectedOptionIds = Object.values(selectedOption).map(op => op.id).sort()
    ...
    useEffect(() => {
        const idsMap = Object.fromEntries(
            Object.entries(selectedOption).map(([typeId, option]: [string, VariationTypeOption]) =>
                [typeId, option.id])
        )
    ...
    const chooseOption = (
        ...
        setSelectedOption((prevSelectedOptions) => {
            ...
            if (updateRouter) {
                router.get(url, {options: getOptionIdsMap(newOption)}, {
                    preserveScroll: true,
                    preserveState: true
                })
            }
                                ...
                                <div key={option.id} onClick={() => chooseOption(type.id, option)}>
                                    {option.images &&
                                        <img src={option.images[0].thumb}
                                             ...
                                             className={"w-[50px] h-auto" + (selectedOption[type.id]?.id === option.id) ? " outline outline-4 outline-primary" : ""}
                                        />}
                                </div>
                            )))}
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="col-span-5">
                        ...
                        {renderProductVariationType()}
                        {
                            ...
                            <div className={"text-error my-4"}>Only {computedProduct.quantity} left</div>
                        }
